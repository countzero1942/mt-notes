...
export const defaultTxConfig: TxConfig = {
  ...
  inline: {
    ...
    hl: {
      ...
      htmlTag: 'mark',
      className: (v) =>
        v ? `tx-highlight tx-hl-${v}` : 'tx-highlight',
      variants: {
        g: 'gold',
        y: 'yellow',
        b: 'blue',
 ...
 */
export function mergeTxConfig(
  userConfig: Partial<TxConfig>
): TxConfig {
  return {
    classPrefix:
      userConfig.classPrefix ?? defaultTxConfig.classPrefix,
    indentUnit:
      userConfig.indentUnit ?? defaultTxConfig.indentUnit,
    scannableMdNodes:
      userConfig.scannableMdNodes ??
      defaultTxConfig.scannableMdNodes,
    poeticTextMode:
      userConfig.poeticTextMode ?? defaultTxConfig.poeticTextMode,
    indentString:
      userConfig.indentString ?? defaultTxConfig.indentString,
    inline: {
      ...defaultTxConfig.inline,
      ...userConfig.inline,
    },
    heading: {
      ...defaultTxConfig.heading,
      ...userConfig.heading,
    },
    block: {
      ...defaultTxConfig.block,
      ...userConfig.block,
    },
  };
}
